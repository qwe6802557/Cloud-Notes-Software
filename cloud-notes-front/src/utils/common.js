export const replaceTargetDomChild = (parentSelector, textContent) => {
    // 找到目标父元素
    const statusLeftEl = document.querySelector(parentSelector);
    if (!statusLeftEl) return;

    const existingFooter = statusLeftEl.querySelector('.editor-footer');

    if (existingFooter) {
        // 存在仅更新文本内容
        const syncTextEl = existingFooter.querySelector('.sync-info > span:last-child');
        if (syncTextEl) {
            syncTextEl.textContent = textContent;
            return;
        }
    }
    ...
    container.className = 'editor-footer';
